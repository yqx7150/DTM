...
from DTM.data.data_util import (paired_paths_from_folder,
                                    paired_DP_paths_from_folder,
                                    paired_paths_from_lmdb,
                                    paired_paths_from_meta_info_file)
from DTM.data.transforms import augment, paired_random_crop, paired_random_crop_DP, random_augmentation
from DTM.utils import FileClient, imfrombytes, img2tensor, padding, padding_DP, imfrombytesDP
...
class DeblurPairedDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        '''
        bb = np.fromfile(gt_path, dtype=np.float32)
        img_gt = bb[0:3*256*256*1].reshape((3,256,256))
        ...
        '''
        aa = np.fromfile(lq_path, dtype=np.float32)
        img_lq = aa[0:3*256*256*1].reshape((3,256,256))
        # augmentation for training
        if self.opt['phase'] == 'train':
            gt_size = self.opt['gt_size']
            # padding
            #img_gt, img_lq = padding(img_gt, img_lq, gt_size)
            #print(img_lq.shape,img_gt.shape,'padding')
            # random crop
            #img_gt, img_lq = paired_random_crop(img_gt, img_lq, gt_size, scale,
            #                                    gt_path)
            #print(img_lq.shape,img_gt.shape,'random crop')   # 512 512 3
            # flip, rotation augmentations
            #if self.geometric_augs:
                #img_gt, img_lq = random_augmentation(img_gt, img_lq)
                #print(img_lq.shape,img_gt.shape,'flip, rotation augmentations')
        ...
        img_lq = torch.from_numpy(img_lq).cuda()
        img_gt = torch.from_numpy(img_gt).cuda()# 3 256 256
        # normalize
        '''
    ...
    def __getitem__test(self, index):
        ...
        '''
        bb = np.fromfile(gt_path, dtype=np.float32)
        #img_gt = bb[0:144*250*250*1].reshape((144,250,250))#/2.0
        img_gt = bb[188:].reshape((144,250,250))
        #img_gt = bb.reshape((144,250,250))
        lq_path = self.paths[index]['lq_path']
        '''
        img_bytes = self.file_client.get(lq_path, 'lq')
        try:
        ...
        aa = np.fromfile(lq_path, dtype=np.float32)
        #img_lq = aa[0:144*250*250*1].reshape((144,250,250))#/2.0
        img_lq = aa[188:].reshape((144,250,250))
        #img_lq = aa.reshape((144,250,250))
        # augmentation for training
        if self.opt['phase'] == 'train':
            gt_size = self.opt['gt_size']
            # padding
        ...
        img_lq = np.transpose(img_lq,[1,0,2])
        img_gt = np.transpose(img_gt,[1,0,2])
        
        img_lq = torch.from_numpy(img_lq).cuda()
        img_gt = torch.from_numpy(img_gt).cuda()
        # normalize
        '''
        if self.mean is not None or self.std is not None:
            normalize(img_lq, self.mean, self.std, inplace=True)
